...
export const Gamification: React.FC = () => {
  ...
  const getScoreStatus = (score: number) => {
    if (score < 300) return { label: 'Crítico', color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100' };
    if (score < 500) return { label: 'Regular', color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100' };
    if (score < 800) return { label: 'Bom', color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' };
    return { label: 'Excelente', color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' };
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center text-center">
             ...
             <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm mt-2">Você está no caminho certo para a liberdade financeira plena.</p>
          </div>
